...
`META_VSL_10_未視聴_No.01`（`haK9EW1F6dGj`）は即時配信のままではUTAGE APIが条件追加を受け付けなかったため、配信を登録1分後へ変更しました。これにより、未視聴・完了・作成会予約／完了・商談中・次回面談・成約・営業停止の8ラベルを除外条件として反映しています。

本番状態は `reserved` のまま維持しています。新規登録者には入口アクションの後、1分後に動画案内が届きます。
