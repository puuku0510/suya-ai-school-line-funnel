...
| Metaセミナー直行 | `REVIEW_METAセミナー直行_20260805` | 10 | 73 | 83 |
| Instagram 10大特典 | `REVIEW_Instagram10大特典_20260805` | 10 | 79 | 89 |
| 合計 |  | 29 | 230 | 259 |
...
5. 全シナリオCSV: 承認済み259行の機械可読エクスポート
...
```mermaid
flowchart TD
    A["Meta広告専用LINE登録"] --> B["セミナー募集18通"]
    B --> C{"セミナー申込"}
    C -->|"未申込"| B
    C -->|"申込済み"| D["LINE8通・メール4通のリマインド"]
    D --> E{"面談予約"}
    E -->|"予約済み"| F["面談リマインド5通"]
...
flowchart TD
    ...
    C --> D["セミナー募集20通"]
    D --> E{"セミナー申込"}
    E -->|"申込済み"| F["LINE8通・メール6通のリマインド"]
    F --> G["セミナー中QRで特典⑥〜⑩"]
    F --> H{"面談予約"}
    H -->|"予約済み"| I["面談リマインド5通"]
    H -->|"未予約"| J["セミナー翌日10:00 再公開希望確認"]
    J -->|"希望ボタン押下"| K["3日VSL"]
    J -->|"未押下"| L["オプチャまたは終了"]
    K --> M{"未視聴15／途中10／完了10"}
    M -->|"面談予約"| I
...
短い案内動画は挟まない。特典⑤はセミナーへの直接申込案内。特典⑥〜⑩はセミナー中のQRで受け取らせ、LINEでは再配布しない。参加／欠席はVSL入口条件にしない。
...
| アクション | CSVの29アクション行 | 実機確認後決定 | 入口、クリック、完了、期限切れ、予約、営業結果 |
| 配信シナリオ | 本書6章の29シナリオ | 実機確認後決定 | 230メッセージ |
...
| `ig_benefit_1to4_delivered` | 特典①〜④配布 | 原則保持 |

特典⑥〜⑩のLINE配布ラベル・シナリオは作らない。
...
### 6.2 Metaセミナー直行（73メッセージ）

| シナリオ | CSV No. | 通数 | 起点 |
|---|---:|---:|---|
| `META_SEM_10_セミナー募集_未申込者` | 11–28 | 18 | 専用LINE登録 |
| `META_SEM_20_09 申込者LINE（k6x新規）` | 29–36 | 8 | セミナー申込完了 |
| `META_SEM_20_10 申込者メール（k6x新規）` | 37–40 | 4 | セミナー申込完了・メールあり |
| `META_SEM_30_再公開希望確認` | 41 | 1 | 申込済み・面談未予約、翌日10:00 |
| `META_SEM_40_VSL_未開封` | 42–56 | 15 | 再公開希望クリック |
| `META_SEM_40_VSL_一部視聴` | 57–66 | 10 | VSLクリック |
| `META_SEM_40_VSL_視聴完了` | 67–76 | 10 | VSL完了 |
| `META_SEM_80_05 面談予約リマインド（vVq新規）` | 77–81 | 5 | 面談予約完了 |
| `META_SEM_90_オプチャ_初回参加案内` | 82–83 | 2 | 未転換終了 |

必須アクション10行はCSV No.1–10。参加者／欠席者ラベルによる分岐は作らない。

### 6.3 Instagram 10大特典（79メッセージ）

#### 本文の出典ルール

- 59件: 吉本さん原案を基礎に、絵文字・言い回しを維持して使用
- 20件: 吉本さん原案に必要な通数がないため、YouTube承認済み本文で補完
...
| シナリオ | CSV No. | 通数 | 起点 |
|---|---:|---:|---|
| `IG_00_登録直後` | 11 | 1 | 登録直後、特典①〜④ |
| `IG_10_セミナー募集` | 12 | 1 | 登録5分後、特典⑤＝セミナー案内 |
| `IG_10_セミナー募集_未申込者` | 13–32 | 20 | セミナー未申込 |
| `IG_20_09 申込者LINE（k6x新規）` | 33–40 | 8 | セミナー申込完了 |
| `IG_20_10 申込者メール（k6x新規）` | 41–46 | 6 | セミナー申込完了・メールあり |
| `IG_30_再公開希望確認` | 47 | 1 | 申込済み・面談未予約、翌日10:00 |
| `IG_40_VSL_未開封` | 48–62 | 15 | 再公開希望クリック |
| `IG_40_VSL_一部視聴` | 63–72 | 10 | VSLクリック |
| `IG_40_VSL_視聴完了` | 73–82 | 10 | VSL完了 |
| `IG_80_05 面談予約リマインド（vVq新規）` | 83–87 | 5 | 面談予約完了 |
| `IG_90_オプチャ_初回参加案内` | 88–89 | 2 | 未転換終了 |

必須アクション10行はCSV No.1–10。No.11とNo.12の間は必ず5分。短い動画や特典⑥〜⑩のLINE配信を追加しない。
...
- 申込項目: 名前、メールアドレス
- 開催: Zoom
- 申込完了: `seminar_main_registered` 付与、未申込募集停止、LINE8通へ登録、メール取得時のみメール4通へ登録
- 参加／欠席: VSL入口条件に使用しない
- 翌日10:00: 面談未予約者へ再公開希望確認
...
- キャンセル: `roadmap_booked` の扱いと再募集可否を運用担当へ確認
- 無断欠席: `roadmap_noshow` 相当を既存運用から確認。推測で再営業しない
- 面談完了: `sales_won`、`sales_next_meeting`、`sales_no_next_meeting`、`sales_stop` のいずれかを必ず記録
...
3. CSV259行を既存オブジェクトと照合し、`新規`、`更新`、`変更なし` を決める。
...
8. CSVの29アクション、230メッセージ、合計259行と実装件数が一致する。
...
2. 申込完了で未申込18通が止まり、LINE8通・メール4通へ移る。
...
6. 未申込者には7日前10:00・20:00と3日前10:00・20:00の各2通が届き、申込済みには7日前・3日前ともLINEとメールの2接点がある。
7. 申込時点ですでに過ぎている7日前・3日前の申込者リマインドは送らず、申込完了で未申込20通が即停止する。
8. 面談未予約者だけ翌日10:00の再公開希望確認を受け取る。
9. 希望リンク／ボタン押下者だけ3日VSLへ入る。
10. VSL終了後は2回目を送らず、オプチャまたは終了へ進む。
11. 吉本さん原案を割り当てた59件の本文・絵文字がSpreadsheetおよびCSVと一致する。
12. 残り20件は指定済みYouTube補完文であり、新規オリジナル文がない。
13. Drive原本画像35枚（Instagram 13枚＋Metaセミナー直行22枚）が全件存在し、Spreadsheetの対応35行とUTAGEから確認できる。
