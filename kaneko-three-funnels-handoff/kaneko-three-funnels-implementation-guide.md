...
5. 全シナリオCSV: 承認済み263行の機械可読エクスポート
...
flowchart TD
    ...
    B --> C["配布完了＝受取済み／クリック判定なし"]
    C --> D["5分後 特典⑤＝セミナー案内"]
    D --> E["セミナー募集20通"]
    E --> F{"セミナー申込"}
    F -->|"申込済み"| G["LINE8通・メール6通のリマインド"]
    G --> H["セミナー中QRで特典⑥〜⑩"]
    G --> I{"面談予約"}
    I -->|"予約済み"| J["面談リマインド5通"]
    I -->|"未予約"| K["セミナー翌日10:00 再公開希望確認"]
    K -->|"希望ボタン押下"| L["3日VSL"]
    K -->|"未押下"| M["オプチャまたは終了"]
    L --> N{"未視聴15／途中10／完了10"}
    N -->|"面談予約"| J
...
| 配信シナリオ | 本書6章の29シナリオ | 実機確認後決定 | 234メッセージ |
...
必須アクション10行はCSV No.1–10。No.11の特典①〜④配布完了を受取済みの入口とし、実クリックを判定しない。No.11とNo.12の間は必ず5分。旧 BEN-C / BEN-N、短い動画、特典⑥〜⑩のLINE配信を追加しない。
...
3. CSV263行を既存オブジェクトと照合し、`新規`、`更新`、`変更なし` を決める。
...
8. CSVの29アクション、234メッセージ、合計263行と実装件数が一致する。
...
14. 開始10分前の募集案内がイベント開始後に届かない。
15. 面談予約済みのテストユーザーへ予約完了・前日・当日・3時間前・30分前の5通が届き、キャンセルまたは日程変更後の旧予約では止まる。
