...
| Instagram 10大特典 | `REVIEW_Instagram10大特典_20260805` | 10 | 75 | 85 |
| 合計 |  | 29 | 226 | 255 |
...
2. 本書: 状態、分岐、優先順位、停止条件、実装順、テスト
3. 全シナリオCSV: 承認済み255行の機械可読エクスポート
4. UTAGE本番: アカウント、オブジェクトID、イベント、URL、置換文字の実値
...
| 配信シナリオ | 本書6章の29シナリオ | 実機確認後決定 | 226メッセージ |
...
### 6.3 Instagram 10大特典（75メッセージ）

| シナリオ | CSV No. | 通数 | 起点 |
|---|---:|---:|---|
| `IG_00_登録直後` | 11 | 1 | 登録直後、特典①〜④ |
| `IG_10_セミナー募集` | 12 | 1 | 登録5分後、特典⑤＝セミナー案内 |
| `IG_10_セミナー募集_未申込者` | 13–30 | 18 | セミナー未申込 |
| `IG_20_09 申込者LINE（k6x新規）` | 31–38 | 8 | セミナー申込完了 |
| `IG_20_10 申込者メール（k6x新規）` | 39–42 | 4 | セミナー申込完了・メールあり |
| `IG_30_再公開希望確認` | 43 | 1 | 申込済み・面談未予約、翌日10:00 |
| `IG_40_VSL_未開封` | 44–58 | 15 | 再公開希望クリック |
| `IG_40_VSL_一部視聴` | 59–68 | 10 | VSLクリック |
| `IG_40_VSL_視聴完了` | 69–78 | 10 | VSL完了 |
| `IG_80_05 面談予約リマインド（vVq新規）` | 79–83 | 5 | 面談予約完了 |
| `IG_90_オプチャ_初回参加案内` | 84–85 | 2 | 未転換終了 |
...
3. CSV255行を既存オブジェクトと照合し、`新規`、`更新`、`変更なし` を決める。
...
10. 画像、CTA、計測リンク、置換文字、期限を行単位で照合する。
11. テストユーザーで全経路を実行する。
...
8. CSVの29アクション、226メッセージ、合計255行と実装件数が一致する。
...
6. 申込完了で未申込18通が止まり、申込者リマインドへ移る。
7. 面談未予約者だけ翌日10:00の再公開希望確認を受け取る。
8. 希望ボタン押下者だけ3日VSLへ入る。
